...
class Opio_Handler {
    ...
    public function get_business() {
        ...
        if($ent_type == 'allReviewFeed' && $rev_type == 'single') {
            $biz_url = "https://feed.op.io/native/allReviewFeed?entId=${ent_id}";
            $business_string = wp_remote_get($biz_url);
        }
        else if($ent_type == 'reviewFeed' && $rev_type == 'single') {
            $biz_url = "https://feed.op.io/native/reviewFeed?entityid=${ent_id}";
            $business_string = wp_remote_get($biz_url);
        }
            ...
            $biz_url = "https://feed.op.io/native/reviewFeed?entId=${ent_id}";
            $business_string = wp_remote_get($biz_url);
        }
        else if($ent_type == 'reviewFeed' && $rev_type == 'orgfeed') {
            $biz_url = "https://feed.op.io/native/multiReviewFeed?orgId=${org_id}&schema_enabled=true&schema_type=Local Business";
            $business_string = wp_remote_get($biz_url);
        }
        else if($ent_type == 'allReviewFeed' && $rev_type == 'orgfeed') {
            $biz_url = "https://feed.op.io/native/multiReviewFeed/allReviews?orgId=${org_id}&schema_enabled=true&schema_type=Local Business";
            $business_string = wp_remote_get($biz_url);
        }
        else {
